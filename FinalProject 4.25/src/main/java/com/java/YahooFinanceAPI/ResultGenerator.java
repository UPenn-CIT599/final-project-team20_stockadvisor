...
//        System.out.println(m.getBenchmark(tickerInput));
        ...
        if (benchmarkSlope >= clientSlope) {
            recommendation = "We rate "+ tickerInput.toUpperCase() +" as "+"Underweight and view it as a core holding given its weak competitive position in "+termSelection + " across most key secular growth segments in its industry and over perform the selected benchmark (S&P 500)." +"\n"+"\n"+ footnote;
            }
        else {
