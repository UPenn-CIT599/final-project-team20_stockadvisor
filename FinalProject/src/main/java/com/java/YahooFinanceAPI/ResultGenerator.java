...
public class ResultGenerator {
    ...
    public String process(String tickerInput, String term) {
        ...
        if (benchmarkSlope >= clientSlope) {
            recommendation = "We rate " + tickerInput.toUpperCase() + " as "
                    + "Underweight and view it as a core holding given its weak competitive position in "
                    + termSelection
                    + " across most key secular growth segments in its industry and under perform the selected benchmark (S&P 500)."
                    + "\n" + "\n" + footnote;
        }
        else {
            recommendation = "We rate " + tickerInput.toUpperCase() + " as "
                    + "Overweight and view it as a core holding given its strong competitive position in "
                    + termSelection
                    + " across most key secular growth segments in its industry and over perform the selected benchmark (S&P 500)."
                    + "\n" + "\n" + footnote;
        }
